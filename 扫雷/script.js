...
function handleExplodeAction(row,col,game,cellEl,tableEl){
    ...
    panelEl.innerHTML=` <h3>你输啦~用了${game.seconds}秒...</h3>`;
...
function handleWin(row,col,game,cellEl,tableEl){
    ...
    gameEl.append(panelEl);
    panelEl.innerHTML=` <h3>成功啦！mua！用了${game.seconds}秒哟~</h3>`;
...
function renderWelcome(){
    ...
    buttonEl=gameEl.querySelector('button#media');
    buttonEl.onclick=()=>{    
        let game = new Game(12, 18, 0.15);
    game.shuffle();
    renderTable(game);
}
